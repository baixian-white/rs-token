"""E41 — Drive 3 direct-quantize-RemoteCLIP trainings (seeds 41, 42, 43).

Each ckpt: ~50–60 minutes (faster than RS-Token because the encoder is frozen).
3 ckpts ≈ 3 hours wall-clock.

Skip behavior: a run whose `best.pt` already exists is skipped.

Usage:
    python scripts/run_e41_direct_clip.py
"""
...
def main() -> int:
    ...
    print(f"E41 direct-quantize-RemoteCLIP driver — {len(runs)} runs, sequential",
          flush=True)
    ...
    for idx, (name, cfg, ck) in enumerate(runs, start=1):
        best = ck / "best.pt"
        log_path = RUN_LOG / f"e41_train_{name}.log"
        if best.exists():
        ...
        cmd = [
            PY, str(ROOT / "scripts" / "train_direct_clip.py"),
            "--config", cfg_rel,
        ]
